...
export default function ProductDetails() {
  ...
  const handleQty = (type: "inc" | "dec") => {
    setQuantity((prev) => {
      if (type === "inc") return prev < maxQty ? prev + 1 : prev;
  ...
  const isInWishlist = () => {
    return wishlist?.wishlist?.products?.some(
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-4 sm:mt-5">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 sm:px-6 py-2 rounded border transition"
              style={{
                borderColor: isInWishlist() ? "red" : "#ec4899",
                color: isInWishlist() ? "red" : "#ec4899",
